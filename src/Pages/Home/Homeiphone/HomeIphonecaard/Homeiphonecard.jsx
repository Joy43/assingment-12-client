  ...
  return (
    <div className="max-w-sm rounded overflow-hidden bg-[#FFFFFF] shadow-2xl hover:shadow-xl">
      ...
      <div className="px-6 py-4">
        ...
        <div className="flex justify-between">
          ...
          <div className="flex items-center bg-slate-400 p-1">
            <img
              src="https://img.icons8.com/color/24/000000/up-arrow.png"
              className="mr-2"
            />
      ...
      <div className="flex items-center gap-x-2">
        ...
        <div>
          <h3 className="text-xl font-bold text-gray-50">{product.brand}</h3>
          <span className="text-xs text-gray-300">New location, USA</span>
        </div>
      </div>
      <div className="my-4">
        <h3 className="text-2xl font-medium text-gray-200">{product.name}</h3>
        <div className="text-sm font-medium">
          <span className="m-1 ml-0 inline-block text-blue-500">{product.rating}</span>
          
          <span className="m-1 ml-0 inline-block text-pink-500">Technology</span>
          <span className="m-1 ml-0 inline-block text-yellow-500">{product.price}$</span>
          <span className="m-1 ml-0 inline-block text-blue-500">{product.brand}</span>
        </div>
        <div className="mt-2 text-sm text-gray-400">{product.description} this day offer</div>
      </div>
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium text-gray-50">stock: {product.stock}</span>
        <a className="font-medium text-blue-500 transition-all duration-300 group-hover:text-blue-500/80">{product.category}</a>
      </div>
   <div className="text-center items-center justify-center">
   <button   onClick={closeModal} type="button" className="focus:outline-none text-red-600 text-sm py-2.5 px-5 rounded-md border border-red-600 hover:bg-red-50 flex items-center">
